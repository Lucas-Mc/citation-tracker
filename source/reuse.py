...
def get_all_info(id, email):
    ...
    try:
        ...
        pmid = result['PubmedArticle'][0]['MedlineCitation']['PMID']
        IS_BOOK = False
    except IndexError:
        # Book instead of article
        current_citation = result['PubmedBookArticle'][0]['BookDocument']['Book']
        pmid = result['PubmedBookArticle'][0]['BookDocument']['PMID']
        IS_BOOK = True
    temp_df['Title'] = get_title(id, email)
    temp_df['URL'] = f'https://pubmed.ncbi.nlm.nih.gov/{pmid}/'
    try:
        if not IS_BOOK:
    ...
    if not IS_BOOK:
        try:
            ...
            # See if there is another way to extract the authors
            print(current_citation)
            author_list = [{
                'ForeName': 'XX',
                'LastName': 'XX',
                'AffiliationInfo': [{'Affiliation': 'XX'}]
            }]
    else:
